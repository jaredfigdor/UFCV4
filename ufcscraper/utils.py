...
def links_to_soups(
    ...
    concurrently when n_sessions > 1, or processes sequentially when n_sessions = 1
    to avoid multiprocessing issues on Windows.
    ...
    """
    # Handle single session case without multiprocessing to avoid Windows issues
    if n_sessions == 1:
        session = get_session()
        try:
            for url in urls:
                try:
                    soup = link_to_soup(url, session, delay)
                    yield (url, soup)
    ...
    # Original multiprocessing implementation for n_sessions > 1
