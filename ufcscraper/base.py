...
class BaseFileHandler(ABC):
    ...
    def remove_duplicates_from_file(self) -> None:
        ...
        # Try reading with different encodings to handle Unicode issues
        for encoding in ['utf-8', 'utf-8-sig', 'latin1', 'cp1252']:
            try:
                ...
                logger.warning(f"Failed to read {self.data_file} with {encoding} encoding, trying next...")
                continue
        else:
            # If all encodings fail, try with error handling
            logger.error(f"Could not read {self.data_file} with standard encodings, using error handling")
            data = pd.read_csv(
                self.data_file,
                dtype=non_date_types,
                parse_dates=date_columns,
